...
param(
  [string]$TaskName = "Poolindex-Backup"
)
...
Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $trigger -Description "Append-only Poolindex snapshot on the Windows control PC" -Force | Out-Null
