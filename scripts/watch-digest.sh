...
try:
    ...
    print(f"watch-digest: SKIP /api/health returned malformed JSON ({e}) — wait for the next daily run", file=sys.stderr)
    sys.exit(1)
resource = d.get("resource") if isinstance(d.get("resource"), dict) else {}
print(resource.get("heavyJobs", 0))
' "$health")"; then
    exit 3
  fi
  if [[ "$heavy" != "0" ]]; then
