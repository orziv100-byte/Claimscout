...
import { billingStatusForUser, cancelUserSubscription } from "@/lib/billing";
...
export async function POST(request: Request) {
  ...
  try {
    await cancelUserSubscription(authed.user.id);
    const body = billingStatusForUser(authed.user.id);
    return withEntitlementCookie(
      NextResponse.json({ ...body, cancelled: true, version: APP_VERSION }),
      { plan: body.plan, wallets: authed.user.wallets },
    );
  } catch (err) {
