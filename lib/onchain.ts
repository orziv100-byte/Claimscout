...
export async function checkEligibility(claimId: string, address: Address): Promise<EligibilityResult> {
  ...
  if (claim.kind === "puzzle") {
    return {
      ...
      status: "unknown",
      detail:
        "Public puzzle documented only. Claimscout will not check keys or attempt a solve. Eligibility is whoever first produces a valid solution — not this wallet check.",
      officialCheckerUrl,
    };
  }
    ...
    return {
      ...
      detail:
        remainingPool && remainingSymbol
          ...
          : "Connect the official checker to see if this address is in the snapshot. Claimscout does not reconstruct merkle proofs.",
      remainingPool,
      remainingSymbol,
      officialCheckerUrl,
    };
